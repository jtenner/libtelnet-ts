...
var telnet;
var ready = bootstrap_1.runtime.then(function (e) {
    telnet = e.instance.exports;
});
var freeIt = function (pointer) { return telnet.free(pointer); };
/**
...
var Telnet = /** @class */ (function (_super) {
    ...
    function Telnet(compatibilityTable, flags) {
        ...
        _this.pointer = telnet.telnet_init(compatibilityTable.pointer, flags); // user data is null
    ...
    Telnet.route = function (telnet, eventPointer) {
    ...
    Telnet.getEvent = function (pointer) {
        return new TelnetEvent_1.TelnetEvent(pointer);
    };
    /**
    ...
    Telnet.prototype.receive = function (bytes) {
        ...
        var bufferPointer = telnet.malloc(bufferLength + 1);
        if (bufferPointer === 0)
            throw new Error("Out of memory.");
        var HEAPU8 = util_1.getHeapU8(telnet);
    ...
    Telnet.prototype.send = function (buffer) {
        ...
        var ptr = telnet.malloc(length);
        util_1.getHeapU8(telnet).set(buffer, ptr);
    ...
    Telnet.prototype.sendText = function (str) {
        var ptr = util_1.writeAsciiToMemory(telnet, str);
    ...
    Telnet.prototype.subnegotiation = function (telopt, data) {
        ...
        var pointer = telnet.malloc(length);
        util_1.getHeapU8(telnet).set(data, pointer);
        telnet.telnet_subnegotiation(this.pointer, telopt, pointer, length);
        telnet.free(pointer);
    };
    /** Begin COMPRESS2. */
    Telnet.prototype.beginCompress2 = function () {
        telnet.telnet_begin_compress2(this.pointer);
    };
    /** Send a ZMP command, and a list of optional arguments. */
    Telnet.prototype.zmp = function (command, args) {
        if (args === void 0) { args = []; }
        var heap = util_1.getDataView(telnet);
        var count = 1 + args.length;
        var argvPointer = telnet.malloc(count << 2);
        var strPtr = util_1.writeAsciiToMemory(telnet, command);
        heap.setUint32(argvPointer, strPtr, true);
        var toFree = [argvPointer, strPtr];
        for (var i = 0; i < args.length; i++) {
            var str = args[i];
            strPtr = util_1.writeAsciiToMemory(telnet, str);
